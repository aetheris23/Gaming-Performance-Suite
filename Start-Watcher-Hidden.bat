...
:: Already running? (pid file present and process alive)
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "Import-Module '.\src\Common.psm1' -Force; $p = Get-WatcherPidFile; if ((Test-Path $p)) { $w = 0; [void][int]::TryParse((Get-Content $p -Raw).Trim(), [ref]$w); if ($w -gt 0 -and (Get-Process -Id $w -ErrorAction SilentlyContinue)) { Write-Host 'Watcher is already running.'; exit 9 } }"
